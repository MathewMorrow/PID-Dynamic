...
float PID_Compute(structPID *PID, float newInput, float newSetpoint, uint32_t newTime)
{
	...
	if(PID->isOn)
	{
		...
		else
		{
			...
			/* Calculate time differential d/dT */
			float deltaT = (float)(newTime - PID->lastTime);
			...
			PID->DtermUnfilt = PID->Kd * (PID->lastInput - newInput) * deltaT;
			/* Filter D term */
